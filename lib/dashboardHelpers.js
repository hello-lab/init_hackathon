...
export async function selectTeamByIdSafe(supabase, teamId, columns) {
  const { data, error } = await supabase
    .from('teams')
    .select(columns)
    .eq('id', teamId)
  ...
  if (error.code === '42703' && columns.includes('is_merged')) {
    const fallbackColumns = removeColumn(columns, 'is_merged')
    ...
    return { data: { ...fallbackData, is_merged: false }, error: null }
  }
...
export async function getTeamData(supabase, teamId) {
  const { data, error } = await selectTeamByIdSafe(
    supabase,
    teamId,
    'id, name, number, owner_id, is_merged, track, created_at'
  )
